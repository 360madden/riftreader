...
public static class TargetCurrentAnchorCacheStore
{
    ...
    public static IReadOnlyList<TargetCurrentAnchorCacheEntry> LoadCandidates(string? filePath, out string? primaryPath, out string? error)
    {
        error = null;
        ...
        var entries = new List<TargetCurrentAnchorCacheEntry>(2);
    ...
    public static string Save(TargetCurrentAnchorCacheDocument document, string? filePath)
    {
        ...
        if (string.IsNullOrWhiteSpace(primaryPath))
        {
            throw new InvalidOperationException("Unable to resolve the target anchor cache path.");
        }
    ...
    private static void TryLoadInto(List<TargetCurrentAnchorCacheEntry> entries, string path, ref string? error)
    {
        if (!File.Exists(path))
        {
            return;
        ...
        try
        {
            ...
            var document = JsonSerializer.Deserialize<TargetCurrentAnchorCacheDocument>(json, JsonOptions);
            if (document is null)
            {
                error ??= $"Target anchor cache file '{path}' did not contain a readable document.";
                return;
            }

            entries.Add(new TargetCurrentAnchorCacheEntry(path, document));
        }
        catch (Exception ex)
        {
            error ??= $"Unable to load target anchor cache '{path}': {ex.Message}";
        }
    }
    ...
    private static string? ResolvePrimaryPath(string? filePath)
    {
        ...
        const string relativePath = @"scripts\captures\target-current-anchor.json";
...
public sealed record TargetCurrentAnchorCacheEntry(
    string Path,
    TargetCurrentAnchorCacheDocument Document);

...
public static class ProcessTargetSignatureScanner
{
    public static TargetSignatureScanResult ScanReaderBridgeTargetSignature(
        ...
        int maxHits)
    {
        ...
        var hits = new List<TargetSignatureScanHit>(coordinateCandidates.Hits.Count);
        ...
        foreach (var candidate in coordinateCandidates.Hits)
        {
            ...
            var signals = new List<TargetSignatureSignal>(8)
            {
            ...
            AddAsciiSignal(bytes, windowStart, candidate.Address, "name", targetName, ref score, 3, signals);
            ...
            hits.Add(new TargetSignatureScanHit(
        ...
        var familySummaries = families
            .Take(maxHits)
            .Select(static family => new TargetSignatureFamilySummary(
        ...
        return new TargetSignatureScanResult(
            Mode: "target-signature-scan",
            ProcessId: processId,
    ...
    private static void AddIntSignals(
        ...
        ref int score,
        int weight,
        List<TargetSignatureSignal> signals,
        int maxOccurrences)
    {
        if (!value.HasValue)
        {
        ...
        for (var index = 0; index < matchIndices.Count; index++)
        {
            ...
            signals.Add(new TargetSignatureSignal(
                Name: signalName,
    ...
    private static string BuildFamilySignature(TargetSignatureScanHit hit)
    {
        return string.Join(
            "|",
            hit.Signals
    ...
    private static string ClassifyFamily(TargetSignatureScanHit representative)
    {
...
public sealed record TargetSignatureScanResult(
    string Mode,
    int ProcessId,
    string ProcessName,
    string SearchLabel,
    ...
    int MaxHits,
    int HitCount,
    IReadOnlyList<TargetSignatureFamilySummary> Families,
    IReadOnlyList<TargetSignatureScanHit> Hits);

public sealed record TargetSignatureFamilySummary(
    string FamilyId,
    string Signature,
    int HitCount,
    int BestScore,
...
public sealed record TargetSignatureScanHit(
    long Address,
    string AddressHex,
    long RegionBase,
    ...
    string FamilyId,
    int FamilyHitCount,
    IReadOnlyList<TargetSignatureSignal> Signals,
    StringHitContext? Context);

public sealed record TargetSignatureSignal(
    string Name,
    string Value,
    int RelativeOffset);

...
public static class DebugTraceWorker
{
    ...
    private static ResolvedBreakpoint ResolveBreakpoint(DebugTraceRequest request, Process process, ProcessMemoryReader reader, List<string> warnings)
    {
        ...
        if (string.Equals(request.Breakpoint.ResolutionMode, "player-coord-trace", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var resolvedModuleOffsetHex = FormatAddress(moduleOffset);
            ...
            if (!string.IsNullOrWhiteSpace(traceDocument.Trace.NormalizedPattern))
            {
                ...
                if (!string.IsNullOrWhiteSpace(scanResult.Address))
                {
                    absoluteAddress = ParseAddress(scanResult.Address);
                }

                if (!string.IsNullOrWhiteSpace(scanResult.RelativeOffsetHex))
                {
                    resolvedModuleOffsetHex = scanResult.RelativeOffsetHex;
                }

                if (!string.Equals(resolvedModuleOffsetHex, FormatAddress(moduleOffset), StringComparison.OrdinalIgnoreCase))
                {
                    warnings.Add(
                        $"Player coord trace module offset drifted from {FormatAddress(moduleOffset)} to {resolvedModuleOffsetHex}; using the live pattern match.");
                }
            }
            ...
            return new ResolvedBreakpoint(
                ...
                ModuleOffsetHex: resolvedModuleOffsetHex,

...
class McpMissionControlTests(unittest.TestCase):
    def test_dashboard_lists_commands_without_starting_public_tunnel(self) -> None:
        ...
        self.assertEqual(payload["finalProductProgress"]["recommendedConnection"], "cloudflare-named-tunnel")
        self.assertTrue(payload["finalProductProgress"]["cloudflareNamedTunnelActive"])
        self.assertTrue(payload["finalProductProgress"]["cloudflareQuickTunnelRetired"])
        self.assertFalse(payload["finalProductProgress"]["cloudflareTunnelRetired"])

#!/usr/bin/env python3
"""Compare +0x158 with camera direction to find offset relationship."""
...
def get_camera_dir():
    dx = read_f32(pid, obj330 + 0x2c)
    dz = read_f32(pid, obj330 + 0x34)
    return math.degrees(math.atan2(dx, dz))

def get_lookahead_facing():
    tx = read_f32(pid, obj90 + 0x14)
    tz = read_f32(pid, obj90 + 0x1c)
    px = read_f32(pid, obj90 + 0x28)
    pz = read_f32(pid, obj90 + 0x30)
...
print("=== Compare all three facing sources ===")
cam = get_camera_dir()
la = get_lookahead_facing()
a158 = get_angle158()
a158_deg = math.degrees(a158)
print(f"Camera direction: {cam:.1f}°")
print(f"Look-ahead target: {la:.1f}°")
print(f"+0x158: {a158:.4f} rad = {a158_deg:.1f}°")
print(f"Camera vs +0x158: {cam - a158_deg:+.1f}°")
print(f"Look-ahead vs +0x158: {la - a158_deg:+.1f}°")
...
print("\n=== Calibration check ===")
print(f"+0x158 to camera: {cam - a158_deg:+.1f}° (should be constant offset)")
print(f"+0x158 to look-ahead: {la - a158_deg:+.1f}° (should be constant offset)")

...
class OperatorLiteTests(unittest.TestCase):
    def test_command_plan_contains_only_safe_default_commands(self) -> None:
        ...
        desktop_repair = next(item for item in plan["commands"] if item["key"] == "desktop-control-repair-guide")
        self.assertIn("riftreader-desktop-control-readiness.cmd", desktop_repair["args"][0])
    ...
    def test_gui_theme_summary_has_distinct_action_groups(self) -> None:
        ...
        self.assertEqual(summary["defaultTheme"], "Dark")
        self.assertIn("Dark", summary["themes"])
        self.assertIn("Light", summary["themes"])
        self.assertIn("Segoe UI", summary["fontFamilies"])
        self.assertIn("Dashboard", summary["tabs"])
        self.assertIn("MCP & Proof", summary["tabs"])
        self.assertIn("Bridge", summary["tabs"])
        self.assertIn("Reports & Git", summary["tabs"])
        self.assertIn("Dashboard", summary["sections"])
        self.assertIn("Bridge", summary["sections"])
        self.assertIn("bridge", summary["buttonVariants"])
        self.assertIn("warning", summary["buttonVariants"])
        self.assertIn("tabbed notebook organization", summary["layoutFeatures"])
        self.assertIn("theme selector dropdown", summary["layoutFeatures"])
        self.assertIn("font selector dropdown", summary["layoutFeatures"])
        self.assertIn("safe command dropdown runner", summary["layoutFeatures"])
        self.assertIn("colorized status output tags", summary["layoutFeatures"])
        self.assertIn("dark and light grouped panels", summary["visualRules"])
        self.assertIn("tabbed MCP proof workspace", summary["visualRules"])
        self.assertIn("dropdown command launcher", summary["visualRules"])
        self.assertIn("runtime font selection", summary["visualRules"])
        self.assertIn("runtime theme selection", summary["visualRules"])
        self.assertIn("colorized output stream", summary["visualRules"])
    ...
    def test_run_command_key_accepts_mcp_helper_aliases(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            proof_packet = operator_lite.run_command_key(root, "proof-run-packet")

...
class RiftReaderChatGptMcpTests(unittest.TestCase):
    def test_manifest_exposes_exact_safe_tool_set_with_annotations(self) -> None:
        ...
        self.assertFalse(annotation_by_name["execute_debugger_ce_action"]["readOnlyHint"])
        self.assertTrue(annotation_by_name["execute_live_control_action"]["destructiveHint"])
        self.assertTrue(annotation_by_name["execute_debugger_ce_action"]["destructiveHint"])
        ...
        self.assertFalse(annotation_by_name["push_current_branch"]["readOnlyHint"])
        self.assertFalse(annotation_by_name["run_bounded_repo_command"]["readOnlyHint"])
        self.assertTrue(annotation_by_name["get_current_head_ci_status"]["readOnlyHint"])
        ...
        self.assertEqual(
            allowed_args_by_name["plan_live_control_action"],
            [
                "actionKind",
                "dryRun",
                "holdMilliseconds",
        ...
        self.assertEqual(
            allowed_args_by_name["execute_live_control_action"],
            [
                "allowMovementRisk",
                "approvalPhrase",
                "dryRun",
        ...
        self.assertEqual(
            allowed_args_by_name["execute_debugger_ce_action"],
            [
                "allowDebuggerRisk",
                "approvalPhrase",
                "dryRun",
        ...
        self.assertEqual(
            allowed_args_by_name["push_current_branch"],
            ["approvalToken", "branch", "expectedHead", "timeoutSeconds", "upstream"],
        )
        self.assertEqual(allowed_args_by_name["get_current_head_ci_status"], ["timeoutSeconds"])
        self.assertEqual(
        ...
        for name, tool_annotations in annotation_by_name.items():
            self.assertEqual(tool_annotations["destructiveHint"], name in {"execute_live_control_action", "execute_debugger_ce_action"})
        for name, tool_annotations in annotation_by_name.items():
            if name in {
                "push_current_branch",
    ...
    def test_push_current_branch_blocks_without_approval_and_does_not_push(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            make_repo(root)
            adapter = make_adapter(root)
            helper_payload = {
                ...
                "preflight": {
                    "status": "ready",
                    "ok": True,
                    "expectedApprovalToken": "PUSH-0123456789abcdef",
                },
                "blockers": ["PUSH_APPROVAL_MISSING"],
                "warnings": [],
                "commands": [],
                "safety": {
                    "gitMutation": False,
            ...
            with mock.patch.object(
                chatgpt_mcp.push_current_branch,
                "push_current_branch_apply",
                return_value=helper_payload,
            ) as push_apply:
                payload = adapter.call_tool(
                    "push_current_branch",
    ...
    def test_get_current_head_ci_status_is_read_only(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            make_repo(root)
            adapter = make_adapter(root)
            helper_payload = {
                ...
                "blockers": [],
                "warnings": [],
                "safety": {"readOnlyGitHubCliInspection": True, "gitMutation": False},
            }
            with mock.patch.object(
                chatgpt_mcp.mcp_ci_status,
                "current_head_ci_status",
                return_value=helper_payload,
            ) as ci_status:
                payload = adapter.call_tool("get_current_head_ci_status", {"timeoutSeconds": 30})

        ci_status.assert_called_once()
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["currentHead"], "a" * 40)
        self.assertFalse(payload["safety"]["gitMutation"])
        self.assertFalse(payload["safety"]["remoteMutation"])
        self.assertTrue(payload["safety"]["readOnlyGitHubCliInspection"])
        assert_repo_root_not_serialized(self, root, payload)

    def test_run_bounded_repo_command_executes_registry_key_only(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            make_repo(root)
            adapter = make_adapter(root)
            helper_payload = {
                ...
                "commandKey": "mcp_server_status",
                "summaryPathRel": ".riftreader-local\\riftreader-chatgpt-mcp\\bounded-commands\\x\\run-summary.json",
                "exitCode": 0,
                "timedOut": False,
                "durationSeconds": 0.1,
                "stdoutPreview": "{\"ok\":true}",
                "stderrPreview": "",
            ...
            with mock.patch.object(
                ...
                payload = adapter.call_tool(
                    "run_bounded_repo_command",
                    {
                        "commandKey": "mcp_server_status",
                        "parameters": {},
                        "expectedRegistryVersion": "bounded-repo-command-registry-v1",
                        "timeoutSeconds": 20,
                    },
                )
        ...
        run_command.assert_called_once_with(
            "mcp_server_status",
            {},
            expected_registry_version="bounded-repo-command-registry-v1",
            timeout_seconds=20.0,
            approval_token=None,
            repo_root=root,
        )
        self.assertTrue(payload["ok"], payload.get("blockers"))
        self.assertEqual(payload["commandKey"], "mcp_server_status")
        self.assertTrue(payload["safety"]["commandExecuted"])
    ...
    def test_run_bounded_repo_command_blocks_unknown_key_without_execution(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            make_repo(root)
            adapter = make_adapter(root)
            helper_payload = {
                ...
                "ok": False,
                "registryVersion": "bounded-repo-command-registry-v1",
                "commandKey": "git_reset_hard",
                "summaryPath": None,
                "exitCode": None,
                "timedOut": False,
                "blockers": ["unknown-command-key:git_reset_hard"],
                "warnings": [],
                "safety": {
            ...
            with mock.patch.object(
                ...
                payload = adapter.call_tool("run_bounded_repo_command", {"commandKey": "git_reset_hard"})
    ...
    def test_get_workflow_control_plan_is_plan_only_and_surfaces_safe_commit_plan(self) -> None:
        ...
        self.assertIn("execute_debugger_ce_action", payload["bidirectionalDataTransfer"]["executeDebuggerCeBoundary"])
        ...
        self.assertEqual(push_contract["status"], "exposed-gated")
        ...
        self.assertEqual(push_contract["currentStage"], 30)
        self.assertEqual(push_contract["exposureStatus"], "exposed-gated")
        self.assertEqual(push_contract["preflightHelper"]["status"], "implemented-local-only")
        ...
        self.assertTrue(push_contract["pushExecutionHelper"]["mcpToolExposed"])
        ...
        self.assertEqual(command_contract["status"], "exposed-gated")
        ...
        self.assertIn("push_current_branch", chatgpt_mcp.TOOL_SPECS)
        self.assertIn("get_current_head_ci_status", chatgpt_mcp.EXPECTED_TOOL_ORDER)
        ...
        self.assertIn("plan_live_control_action", chatgpt_mcp.EXPECTED_TOOL_ORDER)
        ...
        self.assertIn("execute_debugger_ce_action", chatgpt_mcp.EXPECTED_TOOL_ORDER)
        self.assertIn("execute_debugger_ce_action", chatgpt_mcp.TOOL_SPECS)
    ...
    def test_create_fastmcp_server_registers_tools_with_annotations(self) -> None:
        ...
        for registration in server.registrations:
            ...
            self.assertEqual(
                registration["annotations"].destructiveHint,
                registration["name"] in {"execute_live_control_action", "execute_debugger_ce_action"},
            )

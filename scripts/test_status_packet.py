...
class StatusPacketProofFreshnessTests(unittest.TestCase):
    ...
    def test_compact_summary_reports_static_owner_navigation_bridge_commands(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_text(root / "scripts" / "riftreader-phase1-target-entity-snapshot.cmd", "@echo off\n")
        ...
        self.assertTrue(commands["phase1-target-entity-snapshot"]["exists"])
        self.assertIn("post-flush selected-target evidence", commands["phase1-target-entity-snapshot"]["safety"])

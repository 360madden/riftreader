...
class ToolCatalogTests(unittest.TestCase):
    ...
    def test_compact_catalog_contains_workflow_and_static_lane(self) -> None:
        ...
        self.assertTrue(
            any(item["step"] == "postupdate-global-container-coordinate-readback" for item in compact["recommendedWorkflow"])
        )
        ...
        self.assertLess(
            workflow_steps.index("postupdate-static-access-chain"),
            workflow_steps.index("postupdate-global-container-coordinate-readback"),
        )
        self.assertLess(
            workflow_steps.index("postupdate-global-container-coordinate-readback"),
            workflow_steps.index("workflow-status"),
        )
        ...
        self.assertTrue(
            any(
                item["step"] == "navigation-live-run-command-plan-before-live"
                for item in compact["recommendedWorkflow"]
            )
        )
        self.assertTrue(
        ...
        self.assertIn("postupdate-global-container-coordinate-readback", compact["canonicalToolKeys"])
        ...
        self.assertIn("navigation-live-run-command-plan", compact["canonicalToolKeys"])

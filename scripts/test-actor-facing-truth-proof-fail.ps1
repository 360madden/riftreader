...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('RiftReader-actor-facing-truth-proof-fail-' + [System.Guid]::NewGuid().ToString('N'))
...
$mismatchedReaderSource = '0XABCDEF012345'
...
$readerJson = @'
{
  "SelectedSourceAddress": "0xABCDEF012345",
...
try {
    ...
    Assert-True -Condition ($exitCode -ne 0) -Message 'Expected actor-facing truth proof to fail closed on a reader source mismatch.'

    $result = $resultJson | ConvertFrom-Json -Depth 20
    Assert-True -Condition ($result.TruthStatus -eq 'blocked') -Message 'Expected blocked truth status on reader/capture mismatch.'
    Assert-True -Condition (-not $result.LeadMatchesReader) -Message 'Expected the mismatched reader source to break lead-vs-reader parity.'
    Assert-True -Condition (-not $result.CaptureMatchesReader) -Message 'Expected capture-vs-reader parity to fail.'
    Assert-True -Condition (($result.Issues -join "`n") -match 'different live sources') -Message 'Expected the proof result to report the reader/capture source mismatch.'
    Assert-True -Condition (Test-Path -LiteralPath $outputFile) -Message 'Expected blocked truth proof to still write an artifact.'

    Write-Host 'assert-actor-facing-truth fail-closed regression check passed.' -ForegroundColor Green
}
finally {

# Version: riftreader-package-flow-tests-v0.1.4
# Total-Character-Count: 7727
...
class PackageFlowTests(unittest.TestCase):
    ...
    def test_policy_lint_profile_exists(self) -> None:
        self.assertIn("policy-lint", package_flow.PROFILE_COMMANDS)
        expected = package_flow.profile_expected_files("policy-lint")
        self.assertIn("tools/riftreader_workflow/policy_lint.py", expected)
        commands = package_flow.profile_commands("policy-lint")
        flat = [" ".join(command) for command in commands]
        self.assertTrue(any("policy_lint.py --json validate-repo --scope changed" in item for item in flat))

    def test_infer_policy_lint_profile(self) -> None:
        profile = package_flow.resolve_profile("auto", None, {"package": "RiftReader_PolicyLint_v0.1.2"})
        self.assertEqual(profile, "policy-lint")

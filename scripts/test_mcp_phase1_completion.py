...
class McpPhase1CompletionTests(unittest.TestCase):
    ...
    def test_repo_side_complete_checks_actual_client_proof_input_when_template_present(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            make_repo(root)
            write_repo_side_artifacts(root)
        ...
        self.assertEqual(
            ...
            [
                ...
                "--check-input",
                "--input",
                r".riftreader-local\riftreader-chatgpt-mcp\proof-input-templates\20260519-010350Z\proof-input.json",
                "--json",
            ],
        )

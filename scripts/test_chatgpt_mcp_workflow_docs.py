...
class ChatGptMcpWorkflowDocsTests(unittest.TestCase):
    ...
    def test_mcp_doc_includes_runtime_and_proof_status_tools(self) -> None:
        text = self.read_doc("docs/workflow/riftreader-chatgpt-mcp.md")
        self.assertIn("39-tool narrow adapter", text)
        ...
        self.assertIn("final 39-tool proof", text)
        self.assertIn("Stage 45 debugger/CE plan-only artifact boundary", text)
        self.assertIn("Canonical 39 allowlisted tool names", text)
        self.assertIn("`toolOutputSchemaCount` | `39`", text)
        self.assertIn("39 allowlisted RiftReader tools", text)
    ...
    def test_stage38_docs_record_no_input_surface_and_historical_consideration_gate(self) -> None:
        ...
        self.assertIn("current ChatGPT Web/Desktop proof contract is 39 tools", live_control)
    ...
    def test_stage41_live_control_design_is_non_executing_and_separates_risks(self) -> None:
        plan = self.read_doc("docs/workflow/riftreader-chatgpt-mcp-50-stage-plan.md")
        live_control = self.read_doc("docs/workflow/riftreader-chatgpt-mcp-live-control-design.md")
        self.assertIn("Post-Stage-45 debugger/CE plan-only surface", plan)
        ...
        self.assertIn("Stage 46 is the next implementation boundary", plan)
        ...
        self.assertIn("Stage 45 debugger/CE plan-only surface complete-local", live_control)
    ...
    def test_stage44_debugger_ce_static_first_design_is_non_executing(self) -> None:
        ...
        self.assertIn("Status: **Stage 45 plan-only surface implemented**", design)
        self.assertIn("does not expose a ChatGPT MCP attach tool", design)
        ...
        self.assertIn("Stage 45 adds only `plan_debugger_ce_action`", final)

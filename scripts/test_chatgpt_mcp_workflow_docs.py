...
class ChatGptMcpWorkflowDocsTests(unittest.TestCase):
    ...
    def test_mcp_doc_includes_runtime_and_proof_status_tools(self) -> None:
        ...
        self.assertIn("Stage 44 debugger/CE static-first design boundary", text)
        self.assertIn("Canonical 38 allowlisted tool names", text)
        self.assertIn("`toolOutputSchemaCount` | `38`", text)
        self.assertIn("38 allowlisted RiftReader tools", text)
    ...
    def test_stage38_docs_record_no_input_surface_and_historical_consideration_gate(self) -> None:
        ...
        self.assertIn("current ChatGPT\nWeb/Desktop proof contract is 38 tools", live_control)
    ...
    def test_stage41_live_control_design_is_non_executing_and_separates_risks(self) -> None:
        plan = self.read_doc("docs/workflow/riftreader-chatgpt-mcp-50-stage-plan.md")
        live_control = self.read_doc("docs/workflow/riftreader-chatgpt-mcp-live-control-design.md")
        self.assertIn("Post-Stage-44 debugger/CE static-first design", plan)
        ...
        self.assertIn("Stage 45 is the next implementation boundary", plan)
        ...
        self.assertIn("Stage 44 debugger/CE static-first design complete-local", live_control)
    ...
    def test_stage44_debugger_ce_static_first_design_is_non_executing(self) -> None:
        ...
        self.assertIn("Status: **Stage 44 complete-local design only**", design)
        self.assertIn("does not expose a\nChatGPT MCP attach tool", design)
        ...
        self.assertIn("No CE/x64dbg tool\nis exposed by Stage 44", plan)
        self.assertIn("does **not** add an approved MCP tool", final)

...
class ChatGptMcpWorkflowDocsTests(unittest.TestCase):
    ...
    def test_mcp_doc_includes_runtime_and_proof_status_tools(self) -> None:
        text = self.read_doc("docs/workflow/riftreader-chatgpt-mcp.md")
        self.assertIn("40-tool narrow adapter", text)
        ...
        self.assertIn("plan_debugger_ce_action", text)
        self.assertIn("execute_debugger_ce_action", text)
        self.assertIn("list_bounded_repo_commands", text)
        self.assertIn("Stage 47 role/auth policy", text)
        self.assertIn("Stage 48 eval suite", text)
        self.assertIn("Stage 49 dashboard/recovery surface", text)
        self.assertIn("authRolePolicy", text)
        self.assertIn("evalSuite", text)
        self.assertIn("riftreader-chatgpt-mcp-eval-suite.cmd", text)
        self.assertIn("--tool-profile public-read-only", text)
        self.assertIn("final 40-tool proof", text)
        self.assertIn("Stage 45 debugger/CE plan-only artifact boundary", text)
        self.assertIn("Canonical 40 allowlisted tool names", text)
        self.assertIn("`toolOutputSchemaCount` | `40`", text)
        self.assertIn("40 allowlisted RiftReader tools", text)
        self.assertNotIn("Canonical 36 allowlisted tool names", text)
        self.assertNotIn("only the 12 allowlisted RiftReader tools", text)
    ...
    def test_stage38_docs_record_no_input_surface_and_historical_consideration_gate(self) -> None:
        ...
        self.assertIn("current ChatGPT Web/Desktop proof contract is 40 tools", live_control)
    ...
    def test_stage41_live_control_design_is_non_executing_and_separates_risks(self) -> None:
        ...
        self.assertIn("Stage 47 role/auth policy metadata", plan)
        self.assertIn("Stage 48 end-to-end eval-suite note", plan)
        self.assertIn("Stage 49 operational dashboard/recovery note", plan)
        self.assertIn("riftreader-chatgpt-mcp-eval-suite.cmd", plan)
        self.assertIn("Eval Suite", plan)
        self.assertIn("personal **No Authentication** lane", plan)
        ...
        self.assertIn("Stage 47 role/auth policy metadata complete-local", live_control)
        self.assertIn("Stage 48 eval-suite\nchecklist complete-local", live_control)
        self.assertIn("Stage 47 adds `authRolePolicy` metadata only", live_control)
        ...
        self.assertIn("| 44 | Debugger/CE static-first design", plan)
        self.assertIn("| complete-local |", plan.split("| 44 |", 1)[1].splitlines()[0])
        self.assertIn("| 45 | `plan_debugger_ce_action` planning tool.", live_control)
        self.assertIn("| 45 | Debugger/CE plan-only surface", plan)
        self.assertIn("| complete-local |", plan.split("| 45 |", 1)[1].splitlines()[0])
        self.assertIn("| 46 | Debugger/CE gated assist", plan)
        self.assertIn("complete-local-fail-closed", plan.split("| 46 |", 1)[1].splitlines()[0])
        self.assertIn("| 47 | Role and auth hardening", plan)
        self.assertIn("complete-local", plan.split("| 47 |", 1)[1].splitlines()[0])
        self.assertIn("| 48 | End-to-end product eval suite", plan)
    ...
    def test_stage44_debugger_ce_static_first_design_is_non_executing(self) -> None:
        plan = self.read_doc("docs/workflow/riftreader-chatgpt-mcp-50-stage-plan.md")
        final = self.read_doc("docs/workflow/riftreader-chatgpt-mcp-final-readiness.md")
        design = self.read_doc("docs/workflow/riftreader-chatgpt-mcp-debugger-ce-static-first-design.md")
        self.assertIn("Stage 44 debugger/CE static-first design note", plan)
        self.assertIn("docs/workflow/riftreader-chatgpt-mcp-debugger-ce-static-first-design.md", plan)
        self.assertIn("Stage 47 role/auth policy metadata implemented", design)
        self.assertIn("Stage 48 eval-suite\nchecklist implemented", design)
        self.assertIn("does not expose a ChatGPT MCP attach backend", design)
        self.assertIn("prefer offline/static evidence first", design)
        self.assertIn("Required Stage 45 plan-only envelope", design)
        self.assertIn("Stage 45 implementation contract", design)
        self.assertIn("plan_debugger_ce_action", design)
        self.assertIn("Required Stage 46 gated-assist preconditions", design)
        self.assertIn("DEBUGGER_STATIC_FIRST_REQUIRED", design)
        self.assertIn("DEBUGGER_CRASH_RISK_NOT_ACKNOWLEDGED", design)
        self.assertIn("noCheatEngine: true", design)
        self.assertIn("x64dbgAttach: false", design)
        self.assertIn("breakpointsSet: false", design)
        self.assertIn("watchpointsSet: false", design)
        self.assertIn("No CE/x64dbg tool", plan)
        self.assertIn("Stage 46 adds `execute_debugger_ce_action`", final)
        self.assertIn("Stage 47 adds `authRolePolicy` metadata", final)
        self.assertIn("Stage 48 adds `tools/riftreader_workflow/chatgpt_mcp_eval_suite.py`", final)
        self.assertIn("Stage 49 surfaces the Stage 48 eval-suite checklist", final)
        self.assertIn("Eval Suite", final)
        self.assertIn("authEnforcementChanged: false", final)
        self.assertIn("get_live_rift_readonly_state", final)
        self.assertIn("plan_live_control_action", final)
        self.assertIn("execute_live_control_action", final)
        self.assertIn("plan_debugger_ce_action", final)
        self.assertIn("execute_debugger_ce_action", final)
        self.assertIn("must block before input because the live backend is unavailable", final)

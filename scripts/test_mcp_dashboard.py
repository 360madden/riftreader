...
class McpDashboardTests(unittest.TestCase):
    ...
    def test_collect_status_marks_local_443_owner_as_diagnostic_only(self) -> None:
        ...
        self.assertEqual(status["evalSuite"]["requiredToolCount"], 40)
        self.assertEqual(status["evalSuite"]["requiredOutputSchemaCount"], 40)
        self.assertEqual(status["evalSuite"]["requiredTransportStatus"], "tool-call-succeeded")
        self.assertIn("--write --summary-md", status["evalSuite"]["writeCommandText"])

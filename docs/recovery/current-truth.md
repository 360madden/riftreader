...
- RRAPICOORD addon-state diagnostics `scripts/captures/rrapicoord-addon-state-diagnostics-20260514-013524-019157/summary.json` checked files only and sent no input/reload/CE/x64dbg. Repo addon contract passed; installed copies under both discovered AddOns roots match the repo required files; `cmd /c scripts\validate-addon.cmd` passed Lua syntax for 4 addons. Therefore the current blocker is not addon install drift; it is that the live runtime `RRAPICOORD1|...source=rift-api...x/y/z...savedVariablesUse=none` marker is not observed/refreshing in the current session.
- Latest post-addon-state milestone review `scripts/captures/riftscan-milestone-review-20260514-013621.json` is unchanged: it routes the selected family candidate to read-only proof only and keeps `movementAllowedByReview=false`. This does not override the fresh-reference blocker.

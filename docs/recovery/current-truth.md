...
| Broader high-heap scan | Range-split aligned scans found additional exact `RRAPICOORD1`-like high-heap families outside the unaligned ChromaLink destination page. Review artifact: `scripts/captures/high-heap-coordinate-family-review-currentpid-60628-20260513-0637/summary.json`; `candidateCount=53`, `familyCount=24`. Best exact candidates include `0x1FF0D0860A0`, `0x1FF0AC96910`, `0x1FF0BB7D430`, `0x1FF0C3BC740`, and others with value `[7406.1298828125, 871.7699584960938, 3028.77001953125]` and max abs delta `4.1503906231810106e-05`. These are still candidate-only and may include scene/player-info/render copies. |
| Displaced-pose blocker | C# exact-HWND SendInput successfully targeted foreground RIFT, but fresh references did not change after `W`/`w` stimuli. Artifact: `scripts/captures/movement-stimulus-displacement-check-currentpid-60628-20260513-0642/summary.json`; status `blocked-no-displaced-pose`. Do not spam movement; next displaced-pose ranking needs manual displacement or visual/state diagnosis. |
...
| Source-buffer/copy-path lead | At coordinate-copy/read contexts, `rdx = 0x1FF6D600020`, `r14 = 0x1FF65FADE88`, and source buffer `rdx + 0x28` tracked current XYZ across two displaced copied poses: `[7406.6005859375, 871.7725830078125, 3028.814208984375]` and `[7406.1318359375, 871.7725830078125, 3028.77099609375]`. Disassembly of `rift_x64.exe+0x47D408` shows this is a ring/copy routine: `rdi=rcx`, `r15=rdi+0x50`, `r14=[rdi]`, destination `r12=[r15]+[rdi+0x94]+[rdi+0x9c]`, source `rdx=[r14]`. This explains why the source lead is heap-local and not yet a static player-coordinate chain. |
...
| Pointer-chain status | Latest bounded depth-4 pointer scan `scripts/captures/pointer-family-scan-20260513-061835-695118/summary.json` scanned `25` target/owner leads and found heap references only. Total module hits `0`; total `rift_x64.exe` hits `0`. `0x1FF6D600020` is referenced by heap-local storage; `0x1FF65FADE88` is referenced by `0x1FF6D658590`; no restart-stable static pointer chain. |

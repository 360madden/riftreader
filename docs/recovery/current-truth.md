_Last updated: 2026-05-14 01:15 UTC. Current live target remains `rift_x64` PID `2928`, HWND `0xC0994`, process start `2026-05-13T16:17:56.208370Z`, module base `0x7FF71CD90000`. Fresh RRAPICOORD reference capture has returned and the one-command coordinate proof preflight passed read-only proof readiness at `scripts/captures/coordinate-proof-preflight-20260514-003348-430776/summary.json`; movement remains **blocked**. The newest strongest candidate evidence is `family-snapshot-hit-000002` at `0x268E113FED0`, which read back stably against fresh RRAPICOORD reference with `ReferenceMatchCount=1` and max abs delta `0.00004882812481810106` at `scripts/captures/riftscan-proof-pose-20260514-004221/riftscan-riftreader-currentpid-2928-readback-wrapper-summary-20260513-204222.json`. This is still candidate-only: proof-anchor resolution fails against the old PID `57656` / HWND `0x5417BC` cache, there is no restart proof, and grouped pointer scans found heap-only refs with `0` module/RIFT-module hits. Do not navigate or poll movement from this candidate until a current-process proof anchor/static chain passes the same-target ProofOnly gate._

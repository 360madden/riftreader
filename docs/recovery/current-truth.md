...
_Last updated: May 8, 2026 04:18 EDT / 08:18 UTC (latest live validation refresh; live proof snapshot from `rift_x64` PID `33912`, HWND `0xE0DB2`). Current-session movement truth remains **movement-grade and 2m waypoint-smoke validated**, but auto-turn is still **blocked**. We found and fixed the foreground SendInput helper bug: `post-rift-key.ps1` had a 32-byte INPUT struct and now uses the correct 40-byte INPUT union. After the fix, true foreground SendInput `d` delivered without AutoHotkey fallback but still no-turned at 125ms and twice at 500ms; post-message `d` remains inconsistent/opposite-sign. Post-profile `ProofOnly` passed. No CE was used, and SavedVariables were used only as intentionally refreshed post-save snapshots, never as live IPC._
...
| Latest no-input proof | Post-SendInput-fix-profile `ProofOnly` passed on current PID `33912` / HWND `0xE0DB2`; summary `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-081803\run-summary.json`; readback `C:\RIFT MODDING\RiftReader\scripts\captures\proof-anchor-currentpid-33912-readback-summary-20260508-041827.json`; `movementSent=false` |
...
| Latest runtime progress | `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-081803\run-progress.json`; status `passed-proof-only`; `movementSent=false` |
| Latest runtime pointer | `C:\RIFT MODDING\RiftReader\scripts\captures\latest-live-test-run.json` points to post-SendInput-fix-profile `ProofOnly` run `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-081803\run-summary.json` |
...
| Current coordinate | Post-SendInput-fix-profile `ProofOnly` coordinate `X=7436.64013671875`, `Y=885.2191772460938`, `Z=3055.749267578125` at `2026-05-08T08:18:31.5159234Z` |
| Latest proof anchor | Current PID `33912` movement gate was satisfied at latest post-SendInput-fix-profile proof; proof is age-gated, so re-bind exact target and run fresh preflight before more movement |
| Latest validation | May 8 04:18 EDT: fixed `post-rift-key.ps1` SendInput INPUT-size bug (`32 -> 40` bytes); post-fix true foreground SendInput `d` delivered without AutoHotkey fallback but yaw delta stayed `0.0` at 125ms and two 500ms attempts, zero proof-coordinate delta, no promoted backend; post-message `d` remains inconsistent/opposite-sign; no CE and no SavedVariables live truth |
...
| Latest proof | `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-081803\run-summary.json`; current coordinate `7436.64013671875,885.2191772460938,3055.749267578125`; `movementSent=false` |
| Visual verification | `wait_for_frame_change` returned changed after live profiles, including `0.7795%` after the post-fix 125ms true SendInput `d` probe and `2.0506%` after the post-fix 500ms true SendInput `d` probe; screenshots include `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-041459-782.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-041505-426.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-041748-159.png`, and final capture `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-041755-916.png` |
| Safety | no CE; exact PID/HWND; no SavedVariables live truth; profiler detected no proof-coordinate movement during key attempts including arrows, longer `d`, and post-fix true SendInput `d`; no forward navigation was sent |
| Next boundary | Auto-turn remains blocked. Post-message and foreground SendInput `d`/`a`, foreground SendInput `Left`/`Right` arrows, longer 500ms `d` trials, and post-fix true SendInput `d` trials are not promoted; do not re-run auto-turn-to-forward until a backend is promoted by at least two same-sign yaw deltas with zero proof-coordinate movement. |

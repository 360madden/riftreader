...
_Last updated: May 8, 2026 03:55 EDT / 07:55 UTC (latest live validation refresh; live proof snapshot from `rift_x64` PID `33912`, HWND `0xE0DB2`). Current-session movement truth remains **movement-grade and 2m waypoint-smoke validated**, but auto-turn is still **blocked**: the Python turn-key profiler verified exact target and profiled post-message/foreground `d`/`a` plus foreground `Left`/`Right` arrows, yet no key/backend was promoted as repeatable. Latest `ProofOnly` passed before the foreground `Right` arrow probe; post-profile visual frame comparison changed `6.552%`. No CE was used, and SavedVariables were used only as intentionally refreshed post-save snapshots, never as live IPC._
...
| Latest no-input proof | Post-arrow-turn-key-profile `ProofOnly` passed on current PID `33912` / HWND `0xE0DB2`; summary `C:\RIFT MODDING\RiftReader\scripts\captures\turn-key-profile-currentpid-33912-20260508-075123\proof-refreshes\002-pwsh-foreground-sendinput-Right-r1\live-test-ProofOnly-20260508-075225\run-summary.json`; readback `C:\RIFT MODDING\RiftReader\scripts\captures\proof-anchor-currentpid-33912-readback-summary-20260508-035250.json`; `movementSent=false` |
...
| Latest runtime progress | `C:\RIFT MODDING\RiftReader\scripts\captures\turn-key-profile-currentpid-33912-20260508-075123\proof-refreshes\002-pwsh-foreground-sendinput-Right-r1\live-test-ProofOnly-20260508-075225\run-progress.json`; status `passed-proof-only`; `movementSent=false` |
| Latest runtime pointer | `C:\RIFT MODDING\RiftReader\scripts\captures\latest-live-test-run.json` points to post-arrow-turn-key-profile `ProofOnly` run `C:\RIFT MODDING\RiftReader\scripts\captures\turn-key-profile-currentpid-33912-20260508-075123\proof-refreshes\002-pwsh-foreground-sendinput-Right-r1\live-test-ProofOnly-20260508-075225\run-summary.json` |
...
| Current coordinate | Post-foreground-arrow-profile `ProofOnly` coordinate `X=7436.64013671875`, `Y=885.2191772460938`, `Z=3055.749267578125` at `2026-05-08T07:52:54.9073506Z` |
| Latest proof anchor | Current PID `33912` movement gate was satisfied at latest post-foreground-arrow-profile proof; proof is age-gated, so re-bind exact target and run fresh preflight before more movement |
| Latest validation | May 8 03:55 EDT: Python turn-key profiler live-tested exact-HWND post-message `a`/`d`, foreground SendInput `d`, and foreground SendInput `Left`/`Right`; `a` no-turned, post-message `d` produced one `-8.384` degree candidate then no-turn repeats, foreground `d` no-turned, arrows no-turned, zero proof-coordinate delta during profiled attempts, no promoted backend; no CE and no SavedVariables live truth |
...
| Latest proof | `C:\RIFT MODDING\RiftReader\scripts\captures\turn-key-profile-currentpid-33912-20260508-075123\proof-refreshes\002-pwsh-foreground-sendinput-Right-r1\live-test-ProofOnly-20260508-075225\run-summary.json`; current coordinate `7436.64013671875,885.2191772460938,3055.749267578125`; `movementSent=false` |
| Visual verification | `wait_for_frame_change` returned changed after live profiles: `45.302%`, `54.2416%`, `60.9831%`, then `6.552%` after the foreground arrow profile; screenshots include `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-033652-882.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-033939-690.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-034629-765.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-035508-125.png`, and final capture `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-035512-416.png` |
| Safety | no CE; exact PID/HWND; no SavedVariables live truth; profiler detected no proof-coordinate movement during key attempts including arrows; no forward navigation was sent |
| Next boundary | Auto-turn remains blocked. Post-message and foreground SendInput `d`/`a` plus foreground SendInput `Left`/`Right` arrows are not promoted; do not re-run auto-turn-to-forward until a backend is promoted by at least two same-sign yaw deltas with zero proof-coordinate movement. |

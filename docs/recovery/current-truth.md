...
_Last updated: May 8, 2026 04:25 EDT / 08:25 UTC (latest live validation refresh; live proof snapshot from `rift_x64` PID `33912`, HWND `0xE0DB2`). Current-session movement truth remains **movement-grade and 2m waypoint-smoke validated**, but auto-turn is still **blocked**. The foreground SendInput helper bug is fixed (`INPUT` size `32 -> 40`) and true SendInput now delivers without AutoHotkey fallback, but `d` and `Left`/`Right` arrows still no-turn under true SendInput. Post-message `d` remains inconsistent/opposite-sign. Post-profile `ProofOnly` passed. No CE was used, and SavedVariables were used only as intentionally refreshed post-save snapshots, never as live IPC._
...
| Latest no-input proof | Post-SendInput-arrow-profile `ProofOnly` passed on current PID `33912` / HWND `0xE0DB2`; summary `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-082438\run-summary.json`; readback `C:\RIFT MODDING\RiftReader\scripts\captures\proof-anchor-currentpid-33912-readback-summary-20260508-042502.json`; `movementSent=false` |
...
| Latest runtime progress | `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-082438\run-progress.json`; status `passed-proof-only`; `movementSent=false` |
| Latest runtime pointer | `C:\RIFT MODDING\RiftReader\scripts\captures\latest-live-test-run.json` points to post-SendInput-arrow-profile `ProofOnly` run `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-082438\run-summary.json` |
...
| Current coordinate | Post-SendInput-arrow-profile `ProofOnly` coordinate `X=7436.64013671875`, `Y=885.2191772460938`, `Z=3055.749267578125` at `2026-05-08T08:25:07.0671213Z` |
| Latest proof anchor | Current PID `33912` movement gate was satisfied at latest post-SendInput-arrow-profile proof; proof is age-gated, so re-bind exact target and run fresh preflight before more movement |
| Latest validation | May 8 04:25 EDT: fixed SendInput helper remains validated (`INPUT=40`); true foreground SendInput `d` and `Left`/`Right` arrows delivered without AutoHotkey fallback but yaw delta stayed `0.0`; one Right-arrow attempt failed closed before input on ProofOnly reference capture, later Right attempt refreshed proof and no-turned; zero proof-coordinate delta in completed attempts; no promoted backend; no CE and no SavedVariables live truth |
...
| Latest proof | `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-082438\run-summary.json`; current coordinate `7436.64013671875,885.2191772460938,3055.749267578125`; `movementSent=false` |
| Visual verification | `wait_for_frame_change` returned changed after post-fix SendInput profiles, including `2.0857%` after the true SendInput arrow profile; latest screenshots include `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-042426-989.png` and final capture `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-042432-595.png` |
| Safety | no CE; exact PID/HWND; no SavedVariables live truth; profiler detected no proof-coordinate movement during key attempts including arrows, longer `d`, post-fix true SendInput `d`, and post-fix true SendInput arrows; no forward navigation was sent |
| Next boundary | Auto-turn remains blocked. Post-message and foreground SendInput `d`/`a`, foreground SendInput `Left`/`Right` arrows, longer 500ms `d` trials, post-fix true SendInput `d` trials, and post-fix true SendInput arrow trials are not promoted; do not re-run auto-turn-to-forward until a backend is promoted by at least two same-sign yaw deltas with zero proof-coordinate movement. |

...
_Last updated: May 8, 2026 04:04 EDT / 08:04 UTC (latest live validation refresh; live proof snapshot from `rift_x64` PID `33912`, HWND `0xE0DB2`). Current-session movement truth remains **movement-grade and 2m waypoint-smoke validated**, but auto-turn is still **blocked**: the Python turn-key profiler verified exact target and profiled post-message/foreground `d`/`a`, foreground `Left`/`Right` arrows, and longer 500ms `d` trials, yet no key/backend was promoted as repeatable. The 500ms post-message `d` trials produced opposite-sign yaw deltas (`+10.2325` then `-10.9922`) with zero coordinate movement; post-profile `ProofOnly` passed. No CE was used, and SavedVariables were used only as intentionally refreshed post-save snapshots, never as live IPC._
...
| Latest no-input proof | Post-long-d-profile `ProofOnly` passed on current PID `33912` / HWND `0xE0DB2`; summary `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-080354\run-summary.json`; readback `C:\RIFT MODDING\RiftReader\scripts\captures\proof-anchor-currentpid-33912-readback-summary-20260508-040417.json`; `movementSent=false` |
...
| Latest runtime progress | `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-080354\run-progress.json`; status `passed-proof-only`; `movementSent=false` |
| Latest runtime pointer | `C:\RIFT MODDING\RiftReader\scripts\captures\latest-live-test-run.json` points to post-long-d-profile `ProofOnly` run `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-080354\run-summary.json` |
...
| Current coordinate | Post-long-d-profile `ProofOnly` coordinate `X=7436.64013671875`, `Y=885.2191772460938`, `Z=3055.749267578125` at `2026-05-08T08:04:21.8201629Z` |
| Latest proof anchor | Current PID `33912` movement gate was satisfied at latest post-long-d-profile proof; proof is age-gated, so re-bind exact target and run fresh preflight before more movement |
| Latest validation | May 8 04:04 EDT: Python turn-key profiler live-tested exact-HWND post-message `a`/`d`, foreground SendInput `d`, foreground SendInput `Left`/`Right`, and longer 500ms `d`; foreground paths no-turned, post-message `d` produced one earlier `-8.384` degree candidate and later opposite-sign `+10.2325/-10.9922` 500ms deltas, zero proof-coordinate delta during profiled attempts, no promoted backend; no CE and no SavedVariables live truth |
...
| Python live-test orchestrator | Run-progress checkpoint validated; plan `C:\RIFT MODDING\RiftReader\docs\live-testing-python-orchestrator-plan.md`; turn-key profiler `C:\RIFT MODDING\RiftReader\scripts\profile_turn_keys.py` added and live-tested |
...
| Foreground `d` probe | `C:\RIFT MODDING\RiftReader\scripts\captures\turn-key-profile-currentpid-33912-20260508-074456\turn-key-profile-summary.json`; exact foreground target, `foreground-sendinput`, input exit `0`, yaw delta `0.0`, proof-coordinate delta `0.0`, so foreground `d` is also not promoted |
| Foreground arrow probe | `C:\RIFT MODDING\RiftReader\scripts\captures\turn-key-profile-currentpid-33912-20260508-075123\turn-key-profile-summary.json`; exact foreground target, `foreground-sendinput`, `Left` and `Right` input exit `0`, yaw delta `0.0` for both, proof-coordinate delta `0.0`, so arrows are also not promoted |
| Longer 500ms `d` probe | `C:\RIFT MODDING\RiftReader\scripts\captures\turn-key-profile-currentpid-33912-20260508-075910\turn-key-profile-summary.json`; foreground SendInput `d` no-turned twice, while post-message `d` produced `+10.232531344273667` then `-10.99219056890891` degree yaw deltas with proof-coordinate delta `0.0`; opposite signs mean no repeatable same-direction backend was promoted |
| Latest proof | `C:\RIFT MODDING\RiftReader\scripts\captures\live-test-ProofOnly-20260508-080354\run-summary.json`; current coordinate `7436.64013671875,885.2191772460938,3055.749267578125`; `movementSent=false` |
| Visual verification | `wait_for_frame_change` returned changed after live profiles: `45.302%`, `54.2416%`, `60.9831%`, `6.552%`, then `50.8006%` after the longer 500ms `d` profile; screenshots include `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-033652-882.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-033939-690.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-034629-765.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-035508-125.png`, `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-040336-284.png`, and final capture `C:\RIFT MODDING\RiftReader\tools\rift-game-mcp\.runtime\screenshots\capture-20260508-040340-508.png` |
| Safety | no CE; exact PID/HWND; no SavedVariables live truth; profiler detected no proof-coordinate movement during key attempts including arrows and longer `d`; no forward navigation was sent |
| Next boundary | Auto-turn remains blocked. Post-message and foreground SendInput `d`/`a`, foreground SendInput `Left`/`Right` arrows, and longer 500ms `d` trials are not promoted; do not re-run auto-turn-to-forward until a backend is promoted by at least two same-sign yaw deltas with zero proof-coordinate movement. |

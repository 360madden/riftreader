# Historical full current truth before trim

> **Historical / stale chronology.** This file preserves the pre-trim `docs/recovery/current-truth.md` for audit history. Do not use it as the live operating truth source. Use `docs/recovery/current-truth.md` and `docs/recovery/current-truth.json` instead.

_Last updated: 2026-05-14 02:13 UTC. Current live target remains `rift_x64` PID `2928`, HWND `0xC0994`, process start `2026-05-13T16:17:56.208370Z`, module base `0x7FF71CD90000`. Movement remains **blocked**. The live API/runtime truth source is repaired for read-only proof: `ReaderBridge` now emits an RRAPICOORD-compatible live marker (`RRAPICOORD1|...source=rift-api...view=Inspect.Unit.Detail(player)...savedVariablesUse=none`) and post-reload preflight `scripts/captures/coordinate-proof-preflight-20260514-020050-354005/summary.json` passed with `readOnlyProofAllowed=true`, `movementAllowed=false`. The newest broad current-PID family snapshot is `scripts/captures/coordinate-family-snapshot-currentpid-2928-20260514-020258/family-import-candidates.json`; the selected proof-backed candidate is `family-snapshot-hit-000001` at `0x268D1FA6120`, with `ReferenceMatchCount=8` and max abs delta `0.00003173828122271516` in readback `scripts/captures/riftscan-proof-pose-20260514-020339/riftscan-riftreader-currentpid-2928-readback-wrapper-summary-20260513-220346.json`. This remains candidate-only: same-target ProofOnly still blocks on missing current proof pointer candidate metadata, pointer/owner scans found heap-only refs with `0` module/RIFT-module hits, no restart/static-chain proof exists, and ChromaLink world-state is still stale/unhealthy. Do not navigate or poll movement from this candidate until a current-process proof anchor/static chain passes the same-target ProofOnly gate with a fresh API/runtime reference._


**May 14 02:00-02:13 UTC live truth repair + fresh family snapshot update:**

- Root cause repair pivot: enabling `RiftReaderApiProbe` in AddonSettings was necessary but not sufficient; even after `/reloadui`, the standalone probe marker remained unobservable. The repaired live truth source is now the already-enabled `ReaderBridge` publish path, which emits `ReaderBridge_RRAPICOORD1` as an RRAPICOORD-compatible runtime marker from live `Inspect.Unit.Detail(player)` data. This is not SavedVariables live truth.
- Code/deploy validation: `cmd /c scripts\validate-addon.cmd` passed; `cmd /c scripts\deploy-addon.cmd` deployed the updated addon source to both discovered AddOns roots; SHA256 for repo and deployed `ReaderBridge.lua` matched after deployment. Exact-target `/reloadui` was sent only to PID `2928` / HWND `0xC0994`; no movement, CE, or x64dbg was used.
- Live marker validation recovered: `scripts/captures/coordinate-proof-preflight-20260514-020050-354005/summary.json` passed (`ready-for-read-only-proof`), RRAPICOORD reference `scripts/captures/rift-api-reference-currentpid-2928-20260514-020051.json` captured `7402.0, 871.78, 3029.4199`, and the freshness watchdog passed at `scripts/captures/reference-freshness-watchdog-20260514-020112-147658/summary.json`. ChromaLink remains stale/unhealthy, so RRAPICOORD is the current proof reference surface.
- Post-repair diagnostics passed: `scripts/captures/rrapicoord-scan-diagnostics-20260514-020126-043153/summary.json` found `1` usable marker, and `scripts/captures/rrapicoord-addon-state-diagnostics-20260514-020130-043012/summary.json` reports `addon-installed-and-live-marker-observed`.
- Adaptive stale-candidate check: the older `family-snapshot-hit-000002` at `0x268E113FED0` no longer matched the new fresh reference in `scripts/captures/riftscan-proof-pose-20260514-020200/`; that proof pose failed with `ReferenceMatchCount=0`. Treat the prior `0x268E113FED0` lead as historical candidate seed evidence only, not current truth.
- Fresh broad family-group snapshot was rerun over `0x268D0000000`-`0x268E8000000` using the repaired live reference. Artifact: `scripts/captures/coordinate-family-snapshot-currentpid-2928-20260514-020258/family-snapshot-summary.json`. It scanned `327,319,552` bytes, found `42,209` plausible triplets and `23` near-reference triplets, and exported `50` import candidates.
- Fresh read-only proof pose against the new candidate file passed at `scripts/captures/riftscan-proof-pose-20260514-020339/`: `ReferenceMatchCount=8`, `StableDecodedCandidateCount=10`, and best candidate `family-snapshot-hit-000001` at `0x268D1FA6120` matched the fresh reference within `0.00003173828122271516` and stayed stable across `4` samples. Movement remains blocked because this is same-pose readback, not a movement-grade proof anchor/static chain.
- Pointer/root follow-up stayed read-only. The generated pointer target list is `scripts/captures/pointer-targets-currentpid-2928-20260514-020339.json`. Bounded pointer scan `scripts/captures/pointer-family-scan-20260514-021115-344001/summary.json` scanned `24` targets; top target was family base `0x268D1FA0000` with `64` heap hits, `0` module hits, and `0` `rift_x64.exe` hits. Owner batch `scripts/captures/pointer-owner-batch-currentpid-2928-20260514-021250-664397/summary.json` inspected `16` owner windows and found `0` module-RVA hints.
- One over-broad depth-2 pointer scan was intentionally stopped after it began expanding too many ref-storage targets; the bounded depth-0 scan above is the current documented pointer-scan evidence. Next pointer work should stay bounded or use a tighter owner/ranking strategy. The helper is now hardened with `--max-total-targets` and `--max-elapsed-seconds` so future depth scans can stop with a summary instead of expanding unchecked.
- Milestone review `scripts/captures/riftscan-milestone-review-20260514-021300.json` selects the new family import candidate file and the new proof-backed candidate `family-snapshot-hit-000001`; it allows read-only proof only and keeps `movementAllowedByReview=false`.

**May 14 00:33-00:44 UTC fresh reference + broad family snapshot update:**

- Coordinate proof preflight passed at `scripts/captures/coordinate-proof-preflight-20260514-003348-430776/summary.json`: RRAPICOORD reference was fresh, `readOnlyProofAllowed=true`, and `movementAllowed=false`. ChromaLink remained stale, but RRAPICOORD was usable and explicitly `savedVariablesUse=none` / no CE / no movement.
- The older same-target synthesized candidate file (`scripts/captures/same-target-candidate-synth-20260513-230531-602926/same-target-candidates.json`) was re-read safely at `scripts/captures/riftscan-proof-pose-20260514-003555/`; it decoded all `3` candidates stably but produced `ReferenceMatchCount=0`. Best near candidate was `same-target-268DF21E6F0-xyz` at `0x268DF21E6F0` with max abs delta `1.7464282226562773`, so the old narrow candidate set remains seed evidence only.
- A broader current-PID family scan over `0x268DD000000`-`0x268E2000000` found `10` near-reference hits at `scripts/captures/family-scan-currentpid-2928-20260514-003713-458360/family-scan-summary.json`; best was `0x268E1122890` with max abs delta `0.600053906249741`.
- A wider coordinate-family snapshot of the same range found the stronger exact-reference candidate `0x268E113FED0` at `scripts/captures/coordinate-family-snapshot-currentpid-2928-20260514-003904/family-snapshot-summary.json` (`8956` plausible triplets, `11` near-reference triplets, best max abs delta about `0.0000488`). The repeat snapshot at `scripts/captures/coordinate-family-snapshot-currentpid-2928-20260514-004131/family-snapshot-summary.json` exported importable candidates at `family-import-candidates.json`; it used the prior fresh reference as a candidate-generation seed only because one intermediate RRAPICOORD refresh attempt was temporarily unavailable.
- Fresh reference capture recovered again at `scripts/captures/manual-reference-check-currentpid-2928-20260513-204202/rift-api-reference-currentpid-2928-20260514-004203.json` (`7402.5898, 871.78, 3028.45`, no CE, no movement, savedVariablesUse none). Using that reference and the new family snapshot candidate file, read-only proof pose `scripts/captures/riftscan-proof-pose-20260514-004221/` captured `ReferenceMatchCount=1`: `family-snapshot-hit-000002` at `0x268E113FED0`, stable across `4` samples, direct max abs delta `0.00004882812481810106`.
...
- Post-fix milestone review `scripts/captures/riftscan-milestone-review-20260514-005443.json` now selects the newest RiftReader family import candidate file and its latest proof-backed candidate `family-snapshot-hit-000002` at `0x268E113FED0` instead of blindly taking the first packet entry. It still allows only read-only proof and keeps `movementAllowedByReview=false`.
- Static/root follow-up stayed read-only. Exact-target preflight passed at `scripts/captures/x64dbg-target-preflight-20260514-005549-731515/summary.json`. Batch owner/ref-storage inspection of `12` heap ref stores wrote `scripts/captures/pointer-owner-batch-currentpid-2928-20260514-005632-012315/summary.json`; it found refs to family bases/prior owner storage but no exact ref to `0x268E113FED0`. Module-hint sweeps for new RVAs `0x263E078`, `0x263E5F8`, `0x26578F8`, and `0x271F748` wrote `root-signature-module-hint-sweep-20260514-005740/005745/005746/005750` artifacts. They produced heap-only/low-score candidates and no static/module root. Treat the new family as strong coordinate candidate evidence, not a stable chain, until access-chain or restart proof exists.
- The one-off owner/ref-storage batch is now repo-owned as `scripts/pointer_owner_batch_inspector.py` / `scripts/rift_live_test/pointer_owner_batch_inspector.py` with tests. A fresh read-only run against `scripts/captures/pointer-family-scan-20260514-004408-853398/summary.json` wrote `scripts/captures/pointer-owner-batch-currentpid-2928-20260514-010553-091229/summary.json`: `15` owner/ref-storage windows inspected, no movement/input/CE/x64dbg attach, top owner-window module RVA hint `0x270FE10` (`3` hits across `3` adjacent ref-storage owners), plus lower single-hit hints `0x2759FA8`, `0x263E5F8`, `0x26AAE70`, `0x272DBC0`, `0x263E950`, `0x2650220`, `0x26502B0`, and `0x26578F8`. Follow-up root-signature sweeps for `0x270FE10`, `0x2759FA8`, `0x2650220`, and `0x26502B0` wrote `root-signature-module-hint-sweep-20260514-010614/010631/010633/010637` artifacts and stayed heap-only/low-score. No static/module root has been promoted.
- Post-milestone strategy review `scripts/captures/riftscan-milestone-review-20260514-010755.json` still selects the proof-backed family candidate for read-only proof only. `movementAllowedByReview=false`; current proof pointer remains `blocked-target-drift`.
- Root-RVA sweep selection is now automated as `scripts/root_signature_batch_sweep.py` / `scripts/rift_live_test/root_signature_batch_sweep.py` with tests. Current read-only run `scripts/captures/root-signature-batch-sweep-currentpid-2928-20260514-011513-452096/summary.json` consumed the owner batch, skipped `10` RVAs, and selected `0` new sweeps because every high-signal hint had already been swept for the current target or was an old root-signature field RVA. This is negative evidence: stop repeating module-hint sweeps unless a new owner batch produces new high-signal RVAs; next escalation should be access-chain tracing or a fresh multi-pose family snapshot, not repeated stale-module probing.
- Freshness regressed after the 00:42 UTC readback. Coordinate proof preflight `scripts/captures/coordinate-proof-preflight-20260514-011824-911933/summary.json` is blocked: ChromaLink world-state is stale/unhealthy, the RRAPICOORD scan found no usable marker, and the previous RRAPICOORD reference is too old (`2861.953>300` seconds). The preflight code now surfaces the RRAPICOORD child command blocker directly as `command-blocked:rrapicoord-reference-scan:rrapicoord-no-usable-marker` so a failed live marker scan cannot be hidden behind stale fallback evidence.
- RRAPICOORD scan diagnostics `scripts/captures/rrapicoord-scan-diagnostics-20260514-012713-317516/summary.json` read only the latest 3 scan artifacts and confirmed the current blocker: 29 hits loaded, 3 `RRAPICOORD1` text hits, 0 marker-like records, and 0 usable markers. Inferred cause is source/error/static text only with no live `source=rift-api`, `view=Inspect.Unit.Detail(player)`, or `x/y/z` coordinate fields. This means the next optimal step is restoring a true live reference surface, not more stale proof/readback attempts.
- Post-diagnostic milestone review `scripts/captures/riftscan-milestone-review-20260514-012910.json` still reports `ready-for-read-only-proof` as a strategy gate for the selected family candidate and `movementAllowedByReview=false`. Treat it as candidate routing only; the stricter proof preflight/fresh-reference gate still blocks proof promotion and movement.
- RRAPICOORD addon-state diagnostics `scripts/captures/rrapicoord-addon-state-diagnostics-20260514-014041-040017/summary.json` checked files only and sent no input/reload/CE/x64dbg. Repo addon contract passed; installed copies under both discovered AddOns roots match the repo required files; `cmd /c scripts\validate-addon.cmd` passed Lua syntax for 4 addons. The stronger blocker is AddonSettings: 6 account-level files were checked and `RiftReaderApiProbe` is missing/not enabled in all 6. This explains why the live runtime `RRAPICOORD1|...source=rift-api...x/y/z...savedVariablesUse=none` marker is not observed/refreshing in the current session.
- Latest post-repair milestone review `scripts/captures/riftscan-milestone-review-20260514-014847.json` is unchanged: it routes the selected family candidate to read-only proof only and keeps `movementAllowedByReview=false`. This does not override the fresh-reference blocker.
- Live truth source repair was applied with `scripts/repair_rrapicoord_addon_settings.py --scope latest --apply` at `scripts/captures/rrapicoord-addon-settings-repair-20260514-014638-634674/summary.json`. It selected `C:\Users\mrkoo\OneDrive\Documents\RIFT\Interface\Saved\rift315.1@gmail.com\AddonSettings.lua`, inserted `RiftReaderApiProbe = "enabled"`, and wrote a backup under `.riftreader-local/backups/rrapicoord-addon-settings/rrapicoord-addon-settings-repair-20260514-014638-634674/`. No movement, input, reloadui, CE, or x64dbg was used.
- Post-repair addon-state diagnostic `scripts/captures/rrapicoord-addon-state-diagnostics-20260514-014646-301342/summary.json` no longer reports the AddonSettings-not-enabled blocker for the active/latest account. Remaining blocker is `current-scan-has-no-usable-rrapicoord-live-marker`, which is expected until the running client reloads/restarts or otherwise loads the newly enabled addon. Post-repair preflight `scripts/captures/coordinate-proof-preflight-20260514-014655-700987/summary.json` and scan diagnostic `scripts/captures/rrapicoord-scan-diagnostics-20260514-014732-931666/summary.json` still block with 0 usable RRAPICOORD markers.

## Superseded historical status from 2026-05-14 00:26 UTC

> Superseded by the 00:45 UTC update above; retained for audit history.

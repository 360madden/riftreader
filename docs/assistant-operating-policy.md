...
| No-movement proof-recovery lane | After current-target proof recovery is authorized, continue through exact target lock, no-input target-control/visual checks, fresh API reference capture, memory inventory, read-only current-PID scans/readbacks, candidate classification, and RiftReader-owned summaries while `movementSent=false`, `inputSent=false`, no debugger/CE, no provider writes, and no truth/proof promotion remain true |
| Movement recommendation point | Recommend controlled displacement stimulus testing after a current-PID candidate file exists and initial API-now vs memory-now deltas are within tolerance; still require explicit approval before sending any movement or game input |
| Separate live gates | Treat `--movement-approved`, `--allow-current-truth-update`, and `--run-proofonly` as independent approvals; do not bundle or infer them from no-movement recovery approval |
...
| Proof-anchor vs actor-chain split | Keep movement-safe proof-anchor recovery separate from player actor static-chain discovery. A current proof/API-family anchor is a safety gate, not a promoted actor chain. Use `docs/recovery/optimized-player-actor-coordinate-chain-workflow.md` for the provenance-first static-chain path |
...
| Actor-chain speed strategy | After a credible current actor-like candidate exists, stop repeating broad value scans by default. Freeze the candidate/owner hypothesis and pivot to owner layout, bounded access provenance, normalized instruction/module/RVA context, resolver generation, API-now vs chain-now validation, and restart validation |
| Escalation after repeated proof-reacquisition failure | Change tactics instead of retrying the same heuristic/bootstrap trace loop. Debugger use must be explicitly authorized for the current turn and scoped to the smallest bounded action that advances the candidate state |
| Bounded access provenance | For static actor-chain discovery, prefer `stop-context` first, verify target responsiveness, then at most one short targeted hardware watchpoint per explicit decision. Do not loop watchpoints, run broad debugger scans, or use memory-access breakpoints by default |

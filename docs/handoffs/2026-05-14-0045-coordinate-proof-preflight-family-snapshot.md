...
_Last updated: 2026-05-14 01:47 UTC._

## Verdict

Fresh RRAPICOORD reference was available for the 00:42 UTC readback, but it is historical now. The active account AddonSettings live-truth repair has been applied for `rift315.1@gmail.com`: `RiftReaderApiProbe = "enabled"` was inserted with a backup. The client has not been reloaded/restarted by this slice, so the newest preflight and RRAPICOORD diagnostics still block because no live `RRAPICOORD1|...source=rift-api...x/y/z...savedVariablesUse=none` marker is observable yet. Movement remains blocked. The strongest candidate is still `family-snapshot-hit-000002` at `0x268E113FED0`; it matched the then-fresh RRAPICOORD reference in readback, but no static/module root, current proof-anchor gate, or fresh current reference exists yet.
...
| `scripts/captures/rrapicoord-addon-state-diagnostics-20260514-014041-040017/summary.json` | Read-only addon-state diagnostic: repo addon contract passed; installed copies under both discovered AddOns roots match repo required files, but AddonSettings checked 6 account files and `RiftReaderApiProbe` was missing/not enabled in all 6. Latest scan diagnostic still has 0 usable markers. |
| `cmd /c scripts\validate-addon.cmd` | Lua syntax validation passed for 4 addons, including `RiftReaderApiProbe`. |
| `scripts/captures/riftscan-milestone-review-20260514-014847.json` | Post-repair strategy gate unchanged: selected family candidate remains read-only proof candidate; `movementAllowedByReview=false`; fresh-reference/preflight blocker still controls. |
| `scripts/captures/rrapicoord-addon-settings-repair-20260514-014638-634674/summary.json` | Applied safe AddonSettings repair to latest active account file `rift315.1@gmail.com\AddonSettings.lua`: inserted `RiftReaderApiProbe = "enabled"`; backup written under `.riftreader-local/backups/rrapicoord-addon-settings/rrapicoord-addon-settings-repair-20260514-014638-634674/`. No game input/reload/movement. |
| `scripts/captures/rrapicoord-addon-state-diagnostics-20260514-014646-301342/summary.json` | Post-repair addon-state diagnostic no longer reports AddonSettings-not-enabled; remaining blocker is live marker not observed because the client still needs addon runtime reload/restart. |
| `scripts/captures/coordinate-proof-preflight-20260514-014655-700987/summary.json` | Post-repair preflight still blocked: RRAPICOORD scan has no usable marker, ChromaLink stale, old RRAPICOORD reference too old. |
| `scripts/captures/rrapicoord-scan-diagnostics-20260514-014732-931666/summary.json` | Post-repair scan diagnostic still has 0 marker-like records / 0 usable markers; expected until the addon is loaded/refreshed in the running client. |

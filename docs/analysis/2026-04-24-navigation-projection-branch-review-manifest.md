...
| `C:\RIFT MODDING\RiftReader\scripts\test-projection-screenshot-gate-workflow.ps1` | Offline validation for parser checks, capture helper build, CMD launcher/target inspection, wrapper key-argument preservation, plan-only no-artifact behavior, analyzer visual-gate smoke, and fail-closed no-screenshot behavior. |
...
Result: `ok=true`; parser, CMD wrapper launcher/target inspection, capture project build,
PowerShell wrapper plan, CMD wrapper plan, wrapper key-argument preservation,
plan-only no-artifact behavior, analyzer visual-gate smoke, and fail-closed

...
| `C:\RIFT MODDING\RiftReader\scripts\test-navigation-projection-offline.ps1` | Branch-level offline validation aggregator for its script/CMD-wrapper contract, the projection workflow validator, Reader tests, and `git diff --check`. |

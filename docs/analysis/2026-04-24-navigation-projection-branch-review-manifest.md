...
| `C:\RIFT MODDING\RiftReader\scripts\test-projection-screenshot-gate-workflow.ps1` | Offline validation for parser checks, capture helper build, shared CMD launcher existence, CMD wrapper shape/launcher/target inspection, wrapper key-argument preservation, plan-only no-artifact behavior, validator CMD-wrapper smoke, analyzer visual-gate smoke, and fail-closed no-screenshot behavior. |
...
Result: `ok=true`; parser, shared CMD launcher existence, CMD wrapper
...
plan-only no-artifact behavior, validator CMD-wrapper smoke, analyzer visual-gate smoke, and fail-closed

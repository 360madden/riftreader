...
| `scripts/capture-chromalink-live-coords.ps1` | Added `-WorldStateUrl` pass-through plus bundle-level `capture-plan.json`, `chromalink-http-bridge-readiness.json`, `chromalink-world-state-contract.json`, `truth-surface.json`, `savedvariables-freshness.json`, `quality-gate.json`, and `artifact-index.json` outputs for world-state captures. |
...
| RiftReader contract preflight against temporary ChromaLink HTTP bridge | passed for `/api/v1` and `/api/v1/riftreader/world-state/schema` with `-SkipWorldState` |

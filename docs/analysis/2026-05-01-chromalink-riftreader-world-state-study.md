...
| `scripts/capture-chromalink-live-coords.ps1` | Added `-WorldStateUrl` pass-through plus bundle-level `chromalink-http-bridge-readiness.json`, `chromalink-world-state-contract.json`, `truth-surface.json`, and `savedvariables-freshness.json` outputs for world-state captures. |

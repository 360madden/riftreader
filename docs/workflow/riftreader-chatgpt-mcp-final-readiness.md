...
| Actual-client proof replay | Latest actual-client proof exists and replays with required proof rules, including `connectionMode=openai-secure-mcp-tunnel`, no public fallback host in primary mode, draft review, and bounded `dryRun.diffPreview` confirmation. | `proof:missing`, `proof:replay-failed` |
...
6. A fresh actual-client proof records `connectionMode=openai-secure-mcp-tunnel`,
   avoids Cloudflare/ngrok fallback hosts in primary mode, and records the same

...
- Browser/Computer command queue contract;
...
| `--desktop-control-repair-guide` | Shortcut for `--run desktop-control-repair-guide`. | Prints the Computer Use native-pipe recovery checklist and safe observation commands; guide-only, no browser/desktop UI automation or repo mutation. |
| `--desktop-control-queue-contract` | Shortcut for `--run desktop-control-queue-contract`. | Prints the inert Browser/Computer command queue contract; plan-only, no queue writes, executor, UI automation, RIFT input, tunnels, package apply, or Git mutation. |
...
| Browser/Computer Readiness | Runs `scripts\riftreader-desktop-control-readiness.cmd --json`. | Read-only readiness report for no-write Browser Use dashboard smoke and Computer Use native-pipe/list-apps smoke; returns exit `2` until external smokes are confirmed. |
| Computer Use Repair Guide | Runs `scripts\riftreader-desktop-control-readiness.cmd --repair-guide --json`. | Guide-only native-pipe recovery checklist, forbidden fallbacks, and safe observation-record commands; no browser/desktop UI automation, RIFT input, tunnels, package apply, or Git mutation. |
| Desktop Queue Contract | Runs `scripts\riftreader-desktop-control-queue-contract.cmd --json`. | Prints the inert queue schema/state-machine contract for future Browser/Computer workflows; execution is disabled and no MCP tool, queue writer, UI automation, RIFT input, tunnel, package apply, or Git mutation is exposed. |
...
| MCP & Proof | Blue MCP proof buttons, amber Browser/Computer readiness/repair, neutral queue-contract, and purple artifact lookup. | Keeps ChatGPT proof, dashboard/browser smoke, and desktop automation readiness visible without adding control endpoints. |

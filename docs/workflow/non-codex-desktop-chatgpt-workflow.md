...
| `scripts\riftreader-mcp-mission-control.cmd --proof-run-packet-md` | Prints a current ChatGPT Web/Desktop proof packet with the live Server URL, backend status, expected current full-tool list, safe ChatGPT call order, and proof check/record commands. |

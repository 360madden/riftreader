  ...
  inputSchema: {
    ...
    params: {
      ...
      properties: {
        mode: {
          type: "string",
          description: "Operation mode: instant (static chain only), verified (API + chain compare), readiness (promotion gates check), or diagnostic (RRAPICOORD health)",
        },
        pid: {
          type: "number",
...
7. If target PID/HWND does not match current-truth, report target-drift blocker.
...
If you are about to run a command that modifies state, STOP and block instead.`,
}

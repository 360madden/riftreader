...
def build_repo_entries(repo_root: Path) -> list[ToolEntry]:
    return [
        ...
        repo_tool(
            repo_root,
            key="navigation-downstream-package",
            label="Navigation downstream package",
            kind="navigation-readback",
            rel_path="scripts/riftreader-navigation-downstream-package.cmd",
            risk="safe-read-only",
            default_use="build a one-command downstream bundle: consumer refresh, consumer demo, route preview, and schema validations",
            allowed=True,
            approval=False,
            command=[
            ...
            notes=[
                "performs read-only target memory read through navigation-consumer-refresh; no input, movement, debugger attach, provider write, or promotion",
                "live route execution remains gated even when the package says a request can be queued",
            ],
        ),
        repo_tool(
            repo_root,
            key="navigation-live-run-request",
            label="Navigation live-run request",
            kind="navigation-report",
            rel_path="scripts/riftreader-navigation-live-run-request.cmd",
            risk="safe-read-only",
            default_use="create a saved gated live-run request artifact from a downstream package without executing movement",
            allowed=True,
            approval=False,
            command=[
            ...
            notes=[
                "saved JSON only; does not invoke route runners, send input, approve movement, attach debugger, write provider data, or promote truth",
                "records intent for later gated review; live execution still requires explicit approval",
            ],
        ),
        repo_tool(
            repo_root,
            key="navigation-live-run-review",
            label="Navigation live-run review",
            kind="navigation-report",
            rel_path="scripts/riftreader-navigation-live-run-review.cmd",
            risk="safe-read-only",
            default_use="review a saved gated live-run request and source package freshness without approving movement",
            allowed=True,
            approval=False,
            command=[
                "scripts\\riftreader-navigation-live-run-review.cmd",
                "--live-run-request-json",
                "<request-summary.json>",
                "--json",
            ],
            notes=[
                "saved JSON only; validates request/package schemas and freshness without target reads or route-runner invocation",
                "executionReviewApproved and executionAuthorized remain false; separate live movement approval is still required",
            ],
        ),
        repo_tool(
            repo_root,
...
def build_recommended_workflow() -> list[dict[str, str]]:
    return [
        ...
        {
            "step": "navigation-downstream-package-for-consumer",
            "command": "scripts\\riftreader-navigation-downstream-package.cmd --waypoint-readiness-json <readiness-summary.json> --json",
        },
        {
            "step": "navigation-live-run-request-for-review",
            "command": "scripts\\riftreader-navigation-live-run-request.cmd --downstream-package-json <package-summary.json> --json",
        },
        {
    ...
    with tempfile.TemporaryDirectory(prefix="riftreader-tool-catalog-selftest-") as temp_name:
        ...
        checks.extend(
            [
                ...
                {
                    "name": "navigation-downstream-package-canonical",
                    "pass": "navigation-downstream-package" in compact["canonicalToolKeys"],
                },
                {
                    "name": "navigation-live-run-request-canonical",
                    "pass": "navigation-live-run-request" in compact["canonicalToolKeys"],
                },
                {

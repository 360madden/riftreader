...
- `C:\RIFT MODDING\RiftReader\scripts\send-rift-key.ps1` - preferred foreground `SendInput` gameplay-key helper; use this when the live client must actually react to a key press
- `C:\RIFT MODDING\RiftReader\scripts\post-rift-key.ps1` / `C:\RIFT MODDING\RiftReader\scripts\post-rift-key.cmd` - compatibility no-focus key helper used by existing discovery harnesses; prefer `C:\RIFT MODDING\RiftReader\scripts\send-rift-key.ps1` for confirmed gameplay input
...
- `C:\RIFT MODDING\RiftReader\scripts\post-rift-thread-command.ps1` / `C:\RIFT MODDING\RiftReader\scripts\post-rift-thread-command.cmd` - experimental `PostThreadMessage` command injection probe against the Rift UI thread; keep as a fallback/reference helper, not the primary command path
...
- `C:\RIFT MODDING\RiftReader\Run-CameraDiscoveryStable.ps1` - preferred root-level camera-discovery entrypoint; runs the safe Alt-S / Alt-Z harnesses and writes durable repo-local artifacts
- `C:\RIFT MODDING\RiftReader\Run-AltZRetry.ps1` - preferred Alt-Z retry wrapper when the first zoom-toggle pass produces weak or partial evidence
- `C:\RIFT MODDING\RiftReader\Run-ManualAltSHoldCaptureFixed.ps1` - preferred manual fallback when a human-held Alt-S capture is needed
- `C:\RIFT MODDING\RiftReader\docs\input-control-workflow.md` - canonical ranking of preferred / situational / experimental Rift input-control scripts plus the recommended action order (Alt-S, Alt-Z, RMB+mouse move, wheel, movement nudges)
